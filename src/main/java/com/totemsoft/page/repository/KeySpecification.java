...
public class KeySpecification implements Specification<Key> {
    ...
    public @Nullable Predicate toPredicate(Root<Key> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
        return findByTagIds(tagIds)
            .and(findByTagTypeIdAndTagTitles(tagTitles))
            .toPredicate(root, criteriaBuilder);
    }

    public static PredicateSpecification<Key> findByTagIds(Set<Long> tagIds) {
    ...
    public static PredicateSpecification<Key> findByTagTypeIdAndTagTitles(Map<Integer, String> tagTitles) {
        return (root, criteriaBuilder) -> {
            final var predicates = new ArrayList<Predicate>();
